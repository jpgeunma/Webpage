...
export default function App() {
    ...
    useEffect(() => {
      const sseForLastMessage = new EventSource(
        `http://localhost:8080/sse-server/sse-server/user/messages?name=${user}`,
        {
      ...
      const sseForAllMessages = new EventSource(
        `http://localhost:8080/sse-server/sse-server/user/messages/all?name=${user}`,
        {
          withCredentials: true,
        }
      );
      ...
      const sseForUsers = new EventSource(
        `http://localhost:8080/sse-server/sse-server/users?name=${user}`,
        {
          withCredentials: true,
        }
      );
    ...
    const handleSubmit = async (event) => {
      ...
      if (nameInputValue && nameInputValue.trim() !== "") {
        ...
        const response = await fetch(
          `http://localhost:8080/sse-server/user?name=${nameInputValue}`,
          requestOptions
        );
